...
const AuthPage = () => {
    ...
    const { control, setValue, getValues,register, handleSubmit, formState: { errors }, watch } = useForm();
    ...
    const onSubmit = async (data) => {
        ...
        const userData = isLogin
            ? { role: data.role, email: data.email, password: data.password, address: data.address, otpToVerify: data.otpToVerify }
            : { role: data.role, name: data.name, email: data.email, password: data.password, roleid: data.roleid, address: data.address, otpToVerify: data.otpToVerify };

        const apiEndpoint = isLogin ? 'login' : 'signup';
        console.log(isLogin ? 'login:' : 'signup:', data.role, userData);

        try {
            const response = await axios.post(`${ApiBaseUrl}/auth/${apiEndpoint}`, userData);
    ...
    const inputFields = [
        ...
        { name: 'otpToVerify', label: 'Otp', type: 'text', required: true },
    ...
    const updatedFilteredFields =
        isLogin
            ? inputFields.filter(field => field.name === 'email' || field.name === 'password' || field.name === 'address' || field.name === 'otpToVerify')
            : inputFields;
    ...
    const sendOtp = async (email) => {
        try {
            ...
            const userdata = { receiver: email };

            const response = await axios.post(`${ApiBaseUrl}/auth/sendOtp`, userdata);
            ...
            console.error('Error sendind otp:', error);
            toast.error(`Error sendind otp`, {
                icon: IoMdCloseCircle,
            });
        }
    }
    ...
    return (
        ...
        <Box mt="0" h="auto" display="flex" justifyContent="center" p="0">
            ...
            <Box borderRadius="20px" 
                ...
                display='flex' w={{base:"95%",md:"55%"}} justifyContent="space-evenly">
                                    ...
                                    <Stack spacing="5">
                                        ...
                                        <Flex justifyContent={"space-between"} gap="10">
                                            <Button variant='brand' onClick={() => {
                                                // Use setValue to get the email entered by the user
                                                const emailValue = getValues('email');
                                                sendOtp(emailValue);
                                            }} w="100%">
                                                Send OTP
                                            </Button>
                                            <Button variant='brand' type="submit" w="100%" >
